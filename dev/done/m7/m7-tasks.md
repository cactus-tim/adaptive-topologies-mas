...
- [x] 2.1 Implement `MeshTopology` — `src/atm/topology/mesh.py`
  - Type: tdd
...
- [x] 2.2 Write unit tests for MeshTopology — `tests/unit/topology/test_mesh.py`
  - Type: tdd
...
- [x] 2.3 Write e2e integration tests for MeshTopology — `tests/integration/topology/test_mesh_e2e.py`
  - Type: integration
...
- [x] 2.4 Create FakeLLM fixtures for Mesh — `tests/fixtures/llm/m7_mesh_consensus.yaml`, `tests/fixtures/llm/m7_mesh_max_rounds.yaml`
  - Type: simple
...
- [x] 4.1 Implement `HierarchicalTopology` — `src/atm/topology/hierarchical.py`
  - Type: tdd
...
- [x] 4.2 Write unit tests for HierarchicalTopology — `tests/unit/topology/test_hierarchical.py`
  - Type: tdd
...
- [x] 4.3 Write e2e integration test for HierarchicalTopology — `tests/integration/topology/test_hierarchical_e2e.py`
  - Type: integration
...
- [x] 4.4 Create FakeLLM fixture for Hierarchical — `tests/fixtures/llm/m7_hierarchical_finalize.yaml`
  - Type: simple
...
## Wave 3: Cross-topology Sanity [Step 5] COMPLETE

- [x] 5.1 Write cross-topology sanity test file — `tests/integration/topology/test_all_topologies_sanity.py`
  - Type: integration
...
- [x] 5.2 Final full suite check
  - Type: simple
  - Acceptance: `uv run pytest tests/ -q` green (892 passed, 32 skipped); `uv run ruff check` clean; `uv run mypy src/atm` has 1 pre-existing error in mesh.py (missing return annotation in _make_agent_node — introduced in Step 2, not in scope of Step 5); все 5 топологий присутствуют в `TopologyRegistry.list_names()`.
