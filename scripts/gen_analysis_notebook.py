...
def _setup_cells() -> list[nbformat.NotebookNode]:
    ...
    return [
        _md(
            ...
            "Fill in `EXPERIMENT_ID`, `PG_DSN`, and `PARQUET_DIR` before running."
        ),
        _code(
            "# Standard imports\n"
            "import pandas as pd\n"
        ...
        _code(
            ...
            "PARQUET_DIR = Path('experiments')  # root directory for parquet artefacts\n"
            "ORACLE_PATH = 'conf/oracle/type_level_manual.yaml'"
        ),
        _code(
            "# Build async session factory from DSN\n"
            "import asyncio\n"
            "from sqlalchemy.ext.asyncio import create_async_engine, async_sessionmaker\n"
            "\n"
            "_engine = create_async_engine(PG_DSN)\n"
            "session_factory = async_sessionmaker(_engine, expire_on_commit=False)\n"
            "\n"
            "# Load experiment metadata and all runs\n"
            "experiment = asyncio.run(load_experiment(EXPERIMENT_ID, session_factory=session_factory))\n"
            "runs_df = asyncio.run(load_runs(EXPERIMENT_ID, session_factory=session_factory))\n"
        ...
        _code(
            ...
            "    load_topology_transitions(EXPERIMENT_ID, session_factory=session_factory)\n"
            ")\n"
            "phases_df = asyncio.run(load_phases(EXPERIMENT_ID, session_factory=session_factory))\n"
            "human_df = asyncio.run(\n"
            "    load_human_interactions(EXPERIMENT_ID, session_factory=session_factory)\n"
            ")\n"
            "llm_df = load_llm_calls_for_experiment(\n"
            "    EXPERIMENT_ID, parquet_dir=PARQUET_DIR\n"
            ")"
        ),
        _code("oracle = load_oracle_table(ORACLE_PATH)"),
    ]
...
def _rq1_cells() -> list[nbformat.NotebookNode]:
    ...
    return [
        ...
        _code(
            "# RQ1c: Phase timeline (Gantt) — pick first run\n"
            "first_run_id = runs_df['id'].iloc[0] if 'id' in runs_df.columns else runs_df['run_id'].iloc[0]\n"
            "fig_timeline = plot_phase_timeline(phases_df, run_id=first_run_id)\n"
            "plt.show()"
        ),
    ]
...
def _rq2_cells() -> list[nbformat.NotebookNode]:
    ...
    return [
        ...
        _code(
            ...
            "# best_static_df must keep 'quality_score' column name for compute_hurt_rate\n"
        ...
        _code(
            "# 5. Guard override rate — fraction of decisions where at least one guard fired\n"
        ...
        _code(
            ...
            "time_per_topo = compute_time_per_topology(transitions_df)\n"
            "print(time_per_topo)"
        ),
        _code(
            "# RQ2 plots\n"
            "\n"
            "# G11a: Transition timeline + quality overlay (pick first run)\n"
            "run_id = runs_df['id'].iloc[0] if 'id' in runs_df.columns else runs_df['run_id'].iloc[0]\n"
            "fig_transitions = plot_transition_timeline_quality(transitions_df, run_id=run_id)\n"
            "plt.show()"
        ),
        _code(
        ...
        _code(
            ...
            "fig_router_cost = plot_router_cost_share(runs_df, llm_df)\n"
            "plt.show()"
        ),
        _code(
            "# G11d: Time per topology\n"
            "fig_time_topo = plot_time_per_topology(transitions_df)\n"
            "plt.show()"
        ),
        _code(
            "# G11e: Oracle gap (LOO) distribution\n"
            "fig_oracle_gap = plot_oracle_gap_loo(runs_df, oracle_gap_loo)\n"
            "plt.show()"
        ),
    ]
